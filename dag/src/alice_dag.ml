...
module Make_ (Name : Name) = struct
  ...
  let roots t = t.roots
  let get_node name t = Name.Map.find name t.nodes_by_name

  let to_string_graph t ~value_to_string =
    ...
    |> List.filter_map ~f:(fun (node : _ Node.t) ->
      ...
      then None
        ...
        let value =
          List.map node.children ~f:(fun (child : _ Node.t) ->
            value_to_string child.value)
          |> String.Set.of_list
        in
        let key = value_to_string node.value in
        Some (key, value)))
    |> String.Map.of_list_exn
  ;;
  ...
  let transitive_closure_multi_in_parent_first_order ~starts =
    let rec loop (node : _ Node.t) seen acc =
      if Name.Set.mem node.name seen
      then seen, acc
      else (
        let seen, acc = loop_multi node.children seen acc in
        let seen = Name.Set.add node.name seen in
        let acc = node :: acc in
        seen, acc)
  ...
  let transitive_closure_in_child_first_order ~start ~include_start =
    let (x :: xs) =
      transitive_closure_multi_in_parent_first_order
        ~starts:(Nonempty_list.singleton start)
    in
    List.rev (if include_start then x :: xs else xs)
  ;;
  ...
  let all_nodes_in_child_first_order t =
    ...
    | None -> []
    | Some roots ->
      transitive_closure_multi_in_parent_first_order ~starts:roots
      |> Nonempty_list.to_list
      |> List.rev
  ;;
